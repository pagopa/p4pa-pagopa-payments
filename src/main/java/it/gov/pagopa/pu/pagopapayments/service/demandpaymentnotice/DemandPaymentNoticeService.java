...
public class DemandPaymentNoticeService {
  ...
  public DemandPaymentNoticeService(OrganizationService organizationService, AuthnService authnService, WorkflowService workflowService,
                                    CieDebtPositionFacadeService cieDebtPositionFacadeService, BrokerService brokerService, @Value("${cie.service-id}") String cieServiceId) {
  ...
  public Triple<DebtPositionDTO, Organization, Broker> handleRequest(PaDemandPaymentNoticeRequest request) {
    ...
    Broker broker = brokerService.getBrokerByBrokerFiscalCode(request.getIdBrokerPA(), accessToken);
    if(broker == null) {
    ...
    if (cieServiceId.equals(serviceId)) {
      return Triple.of(handleCreateCieDebtPosition(request, organization, accessToken),organization, broker);
    } else {
