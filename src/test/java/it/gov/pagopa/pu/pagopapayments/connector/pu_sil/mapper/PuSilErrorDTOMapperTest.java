...
class PuSilErrorDTOMapperTest {

  @Test
  void whenMapThenReturnPuErrorDTO() {
    // Given
    PuSilErrorDTO errorDTO = TestUtils.getPodamFactory().manufacturePojo(PuSilErrorDTO.class);
    Objects.requireNonNull(errorDTO.getFields());

    // When
    PuErrorDTO result = PuSilErrorDTOMapper.map(errorDTO);
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      it.gov.pagopa.pu.pusil.dto.generated.ErrorFieldDTO expectedErrorFieldDTO = errorDTO.getFields().get(i);
